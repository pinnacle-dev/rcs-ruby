...
module Pinnacle
  class ShortenedUrlWithClickData
    # @return [Array<Pinnacle::LinkClickEvent>] Array of click analytics data for this URL. <br>
    ...
    # @param clicks [Array<Pinnacle::LinkClickEvent>] Array of click analytics data for this URL. <br>
    #  Click data will be empty if no one has visited your link yet.
    # @param url [String] Your custom shortened link following the format `https://pncl.to/{linkId}`,
    #  where `{linkId}` is the unique identifier for the URL that can be used with our
    #  other endpoint.
    # @param config [Pinnacle::PinnacleUrlConfig]
    ...
    def self.from_json(json_object:)
      ...
      clicks = parsed_json["clicks"]&.map do |item|
        item = item.to_json
        Pinnacle::LinkClickEvent.from_json(json_object: item)
      end
      url = parsed_json["url"]
      if parsed_json["config"].nil?
        ...
        config = Pinnacle::PinnacleUrlConfig.from_json(json_object: config)
      end
      new(
        clicks: clicks,
        url: url,
    ...
    def self.validate_raw(obj:)
      ...
      Pinnacle::PinnacleUrlConfig.validate_raw(obj: obj.config)
    end
  end
end
