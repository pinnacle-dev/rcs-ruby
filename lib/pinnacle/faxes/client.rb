...
module Pinnacle
  module Faxes
    class Client
      ...
      #
      # **Idempotency:** reuse the same `Idempotency-Key` and identical body after a client timeout. The key is scoped
      # to your team and covers `from`, `to`, `mediaUrl`, and `quality`. Reusing it with an identical body returns the
      # existing logical fax; changing any of those fields returns 409 and never sends another fax.
      #
      # @param request_options [Hash]
